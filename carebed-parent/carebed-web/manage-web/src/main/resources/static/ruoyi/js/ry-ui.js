    ...
    $.extend({
    	...
    	table: {
    		...
    		exportExcel: function(formId) {
    			...
    			$.modal.confirm(remark + table.options.modalName + "吗？", function() {
	    			var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
    		...
    		// 下载模板
    		importTemplate: function(uri) {
    			table.set();
    			window.location.href = ctx + uri;
            },
            // 导入数据
            			...
            			$.ajax({
            				...
            				success: function (result) {
            					if (result.code == web_status.SUCCESS) {
            						...
            						layer.close(index);
            						$.modal.enable();
									var errorContent = "<div>" + result.msg + "</div>";
									var contentList = result.data;
									if($.common.isNotEmpty(contentList) && Array.isArray(contentList)){
										for(var i=0;i<contentList.length;i++){
											errorContent += "<div>" + contentList[i] + "</div>";
										}
									}
									$.modal.alertError("<div style='color:#F00'>" + errorContent + "</div>");
            					}
            				}
            			});
            		}
        ...
        treeTable: {
			...
			hideColumn: function(column, tableId) {
				var currentId = $.common.isEmpty(tableId) ? table.options.id : tableId;
				$("#" + currentId).bootstrapTreeTable('hideColumn', column);
			}
        },
        // 表单封装处理
    	form: {
        ...
        operate: {
			...
			refreshItem: function(id) {
				table.set();
				$.modal.confirm("确定刷新该条" + table.options.modalName + "信息吗？", function() {
			...
			refreshSelect: function() {
				...
				$.modal.confirm("确认要刷新选中的" + rows.length + "条数据吗?", function() {
					var url = table.options.refreshUrl;
					var data = { "codes": rows.join() };
					$.operate.submit(url, "post", "json", data);
				});
			},
            ...
            uphold:function () {
                table.set();
                // $.modal.openUphold("设备编号维护" , "/system/info/uphold","600");
                var _url = "/system/info/uphold";
                var _width = "770";
            ...
            //换卡
            chagecard: function(id) {
            	table.set();
            	if($.table.selectColumns("cardStatus")!="1"){
            		alert("用户卡已注销不能换卡,请新增用户信息");
            		return;
            	}
            	if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
            		var row = $("#" + table.options.id).bootstrapTreeTable('getSelections')[0];
                    ...
                    var url = table.options.chagecardUrl.replace("{id}", row[table.options.uniqueId]);
                    $.modal.open("换卡" + table.options.modalName, url);
            	} else {
            	    $.modal.open("换卡" + table.options.modalName, $.operate.chagecardUrl(id));
            	}
            },
          //金额修改
            chagebalance: function(id) {
            	table.set();
            	if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
            		var row = $("#" + table.options.id).bootstrapTreeTable('getSelections')[0];
                	if ($.common.isEmpty(row)) {
                    ...
                    var url = table.options.chagebalanceUrl.replace("{id}", row[table.options.uniqueId]);
                    $.modal.open("换卡" + table.options.modalName, url);
            	} else {
            	    $.modal.open("换卡" + table.options.modalName, $.operate.chagebalanceUrl(id));
            	}
            },
            // 修改信息，以tab页展现
            editTab: function(id) {
            ...
            // 换卡访问地址
            chagecardUrl: function(id) {
            	var url = "/404.html";
            	if ($.common.isNotEmpty(id)) {
            	    url = table.options.chagecardUrl.replace("{id}", id);
            	} else {
            	    var id = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
            	    if (id.length == 0) {
            			$.modal.alertWarning("请至少选择一条记录");
         ...
         // 余额修改访问地址
            chagebalanceUrl: function(id) {
            	var url = "/404.html";
            	if ($.common.isNotEmpty(id)) {
            	    url = table.options.chagebalanceUrl.replace("{id}", id);
            	} else {
            	    var id = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
            	    if (id.length == 0) {
            			$.modal.alertWarning("请至少选择一条记录");
